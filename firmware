...
contol priority:
(emergency)
S1 pressed = forces window OPEN
S2 pressed = forces window CLOSED
no buttons = automatic temperature control

temp above threshold = open vent(window)
temp below threshold = close vent

The firmware tracks the current ventilation so the servo only activates when a change in window postion is needed.Servo power is supplied through the relay during movement and diconnected 
once the required servo position is met.
